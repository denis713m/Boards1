...
export function* createBoards(action) {
    ...
    try{
        ...
        boards.push({name:action.data.name, user: action.data.user, id:uuidv4()});
        window.localStorage.setItem('boards', JSON.stringify(boards));
        //action.data.history.replace('/');
        yield put({type: types.BOARD_CREATE_SUCCESS, data: {name:action.data.name, user: action.data.user}});
    }
    catch (e) {
        console.log(e);
